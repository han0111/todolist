...
function App() {
  ...
  const [donePlans, setDonePlans] = useState(  
    [{id:1, doneTitle: "리액트공부하기 완료", doneComment: "리액트 기초를 공부해봅시다 완료"},
  ]);
    ...
    <div className="layout">
          ...
          plans.map((item)=>{
            return (
              <div key={item.id} className="component-style">
                <h2>{item.title}{item.doneTitle}</h2>
                <div>{item.comment}{item.doneComment}</div>
                <div className="btn">
      ...
      <div className="app-style">
        {donePlans.map((item)=>{
          return (
            <div key={item.id} className="component-style">
            <h2>{item.doneTitle}{item.title}</h2>
            <div>{item.doneComment}{item.comment}</div>
            <div className="btn">
